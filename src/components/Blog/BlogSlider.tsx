...
const BlogSlider = () => {
  ...
  const loadScripts = async () => {
      ...
      if (typeof window !== 'undefined' && (!window.$.fn || !window.$.fn.owlCarousel)) {
      ...
      // Wait for DOM ready and all scripts/styles loaded
      if (
        typeof window !== 'undefined' &&
        window.$ &&
        window.$.fn &&
        window.$.fn.owlCarousel &&
        document.getElementById('insightsCarousel')
      ) {
        window.$(function () {
          setTimeout(() => {
            // Destroy if already initialized
            if (window.$('#insightsCarousel').hasClass('owl-loaded')) {
              window.$('#insightsCarousel').trigger('destroy.owl.carousel');
              window.$('#insightsCarousel').removeClass('owl-loaded owl-hidden owl-refresh owl-drag');
              window.$('#insightsCarousel').find('.owl-stage-outer, .owl-nav, .owl-dots').remove();
            }
            window.$('#insightsCarousel').owlCarousel({
              loop: true,
              margin: 16,
              dots: true,
              nav: true,
